...
# ---------- Streamlit Config ----------
st.set_page_config(page_title="Breathpod Toolkit", layout="wide")

# ---------- Navigation ----------
page = st.selectbox("🔀 Select Tool", ["Referrals System", "Video Editor (Rotate)"])
...
# ---------- Video Rotation Tool ----------
elif page == "Video Editor (Rotate)":
    st.title("🎬 Video Rotator")

    st.write("Upload one or more videos to rotate 90° anti-clockwise and download them back without quality loss.")

    files = st.file_uploader("Upload video(s)", type=["mp4", "mov", "avi"], accept_multiple_files=True)
    ...
    for uploaded_file in files:
        ...
        output_path = input_path.replace(".mp4", "_rotated.mp4")

        # Run ffmpeg rotation
        try:
            subprocess.run(
                ["ffmpeg", "-i", input_path, "-vf", "transpose=2", "-c:a", "copy", output_path],
                check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )

            # Load rotated file and show download button
            with open(output_path, "rb") as out_file:
                st.download_button(
                    label=f"⬇️ Download Rotated: {uploaded_file.name}",
                    data=out_file,
                    file_name=uploaded_file.name.replace(".mp4", "_rotated.mp4"),
                    mime="video/mp4"
                )

        except subprocess.CalledProcessError as e:
            st.error(f"Failed to process {uploaded_file.name}. Error: {e.stderr.decode()}")
